...
fn sys_mmap(
    ...
    ax_println!("mmap ...");
    if !addr.is_null() {
        ax_println!("addr is not null");
        return -LinuxError::EINVAL.code() as _;
    }
    let range = AddrRange::<VirtAddr>::new(0.into(), 0x4FFFFFFF.into());

    let va = match current()
        .task_ext()
        .aspace
        .lock()
        .find_free_area(0.into(), length, range)
    {
        Some(va) => va,
        None => {
            ax_println!("mmap failed: no free area");
            return -LinuxError::ENOMEM.code() as _;
        }
    };
    ax_println!("mmap va: {:#x?}", va);

    let mut buffer = Vec::new();
    buffer.resize(length, 0);
    let size = sys_read(fd, buffer.as_mut_ptr() as *mut c_void, length as _);

    ax_println!("mmap read size: {}", size);

    let mmapflags = MmapFlags::from_bits_truncate(flags);
    let mapflags = MappingFlags::USER | MappingFlags::READ | MappingFlags::WRITE;

    ax_println!("mmap read buffer: {:?}", buffer);

    match current()
        .task_ext()
        .aspace
        .lock()
        .map_alloc(va, 4096, mapflags, true)
    {
        Ok(_) => {
            ax_println!("mmap success");
        }
        Err(e) => {
            ax_println!("mmap failed: {:?}", e);
            return -LinuxError::ENOMEM.code() as _;
        }
    }

    current().task_ext().aspace.lock().write(va, &buffer);
